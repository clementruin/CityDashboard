...
for i in range(1, 47):    
    ...
    for line in data:
        if line[1].strip() not in idf :
            ...
            if insee_code not in bac_dico.keys() :
                bac_dico[insee_code] = copy.deepcopy(features)
...
### Dictionnaire propre (Clément fdp)
js=[]
for keys in bac_dico.keys():
    js.append({'Code' : keys, 'Results' : bac_dico[keys]})
